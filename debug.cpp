...
template <typename T> void debug(stack<T> st) {
  cerr << " ";
  while (!st.empty()) {
    cerr << debug(st.top()) << " ";
    debug(st.pop());
...
template <typename T> void debug(queue<T> que) {
  while (!que.empty()) {
    cerr << debug(que.front()) << " ";
    debug(que.pop());
  }
}
template <typename T> void debug(deque<T> dque) {
  while (!dque.empty()) {
    cerr << debug(dque.front()) << " ";
    debug(dque.pop_front());
  }
}
void dbg() { cerr << "]\n"; }
template <typename T, typename... V> void dbg(T t, V... v) {
  debug(t);
  if (sizeof...(v))
    cerr << ", ";
  dbg(v...);
...
#ifdef DEBUG
#define dbg(x...)                                                              \
  cerr << "\e[34m" << __func__ << ":" << __LINE__ << " [" << #x                \
       << "]\e[39m = \e[32m[";
dbg(x);
cerr << "\e[39m" << endl;
#else
